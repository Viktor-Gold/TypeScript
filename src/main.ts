...
write_countries.addEventListener('mousedown', (e) => {
    e.preventDefault() // блокирует инпут с помощью mousedown
    e.stopPropagation() // Прекращает дальнейшее всплытие
})
write_countries.addEventListener('input', () => {
    write_countries.addEventListener('keydown', (e:any) => {
        if (e.key >= 0) {
    ...
    for (let i = 0; i < countries.length; i++) {
        if (write_countries.value.toUpperCase() == countries[i][0]) {
            console.log(countries[i]);
            title.innerHTML += `<div>${countries[i]}</div>`
        }
    }
})
